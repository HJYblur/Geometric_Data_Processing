...
def test_mesh_timing():
    # Fix transformation matrix for testing
    translation = mathutils.Matrix.Translation([0.05, -0.03, 0.08])
    rotation = (
        mathutils.Matrix.Rotation(0.02, 4, "X")
        @ mathutils.Matrix.Rotation(-0.015, 4, "Y")
        @ mathutils.Matrix.Rotation(0.01, 4, "Z")
    )
    transformation = translation @ rotation
    ...
    print(
        f"\nTime for registration of size {len(registration_transformations)}: {end_time_1 - start_time_1:.4f} seconds"
    )
    mse_value = calculate_mse(source, destination)
    hausdorff_value = calculate_hausdorff_distance(source, destination)
    print(f"MSE: {mse_value:.5f}, hausdorff_value: {hausdorff_value:.5f}")
